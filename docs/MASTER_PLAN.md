...
### Решение:
1. **User теперь синхронизируется в CloudKit Private Database** в [`AuthViewModel.swift`](FishAndChips/ViewModels/AuthViewModel.swift)
   - При регистрации User сохраняется в CloudKit через `cloudKit.save(record:to:.privateDB)`
   - Проверка CloudKit остаётся для валидации уникальности email
   - Graceful degradation: если CloudKit недоступен - не блокируем регистрацию
...
### Результат:
- User синхронизируется в CloudKit Private Database при регистрации
- Показывается loader во время проверки CloudKit
- После успешной регистрации - уведомление "Вы успешно зарегистрировались!"
- В профиле отображается email рядом с username
- Регистрация работает с проверкой уникальности в CloudKit
